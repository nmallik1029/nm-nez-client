...
import { QUEUE_WINDOW_BACKGROUND, TOKENS_CSS } from '../../shared/ui';
...
.frame{height:100%;display:flex;flex-direction:column;
  border:var(--nm-bw-thick) solid var(--nm-rq-line);background:var(--nm-rq-panel)}
 ...
 */
.frame.live{animation:edge var(--nm-blink) steps(1,end) infinite}
@keyframes edge{
...
#timer{font-size:var(--nm-fs-display);letter-spacing:var(--nm-track-lg);line-height:1;
...
#msg{font-size:var(--nm-fs-xs);letter-spacing:var(--nm-track-lg);color:var(--nm-rq-dim);
  min-height:15px;
  text-align:center}
